...
extern "C"
{
	...
	WXDLLIMPEXP_CL void CLP_CTM_FirstFunctionOfFile(TagEntryPtr& returnTag, const wxString &fileName);
	WXDLLIMPEXP_CL void CLP_CTM_FirstScopeOfFile(TagEntryPtr& returnTag, const wxString &fileName);
	WXDLLIMPEXP_CL void CLP_CTM_FunctionFromFileLine(TagEntryPtr& returnTag, const wxString &fileName, int lineno, bool nextFunction = false);
	...
	WXDLLIMPEXP_CL bool CLP_CTM_GetFunctionDetails(const wxString &fileName, int lineno, TagEntryPtr &tag, clFunction &func);
	WXDLLIMPEXP_CL void CLP_CTM_GetFiles(const wxString &partialName, std::vector<wxFileName> &files);
	WXDLLIMPEXP_CL void CLP_CTM_GetHoverTip(const wxString &fileName, int lineno, const wxString & expr, const wxString &word, const wxString & text, std::vector<wxString> & tips);
	...
	WXDLLIMPEXP_CL void CLP_CTM_TagsFromFileAndScope(const wxString &fileName, const wxString &scopeName, std::vector<TagEntryPtr> &tags);
	WXDLLIMPEXP_CL bool CLP_CTM_WordCompletionCandidates(const wxString &fileName, int lineno, const wxString& expr, const wxString& text, const wxString &word, std::vector<TagEntryPtr> &candidates);
